...
if postgresql.data_directory_empty():
    ...
    #postgresql.write_recovery_conf(etcd.current_leader())
    postgresql.write_recovery_conf({"address": "postgres://198.51.100.42:5432"})
    postgresql.start()
    ...
    if postgresql.is_leader():
        for node in etcd.get_client_path("/members?recursive=true")["node"]["nodes"]:
            ...
            if member != postgresql.name:
                print "created replication slot for %s" % member
